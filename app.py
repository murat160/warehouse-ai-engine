...
st.markdown("<div class='hero'><h1>🌐 Murat AI</h1><p>Простой профессиональный экран: слева вставляешь видео, аудио, ссылку или текст — справа получаешь готовое видео, озвучку или текст.</p></div>", unsafe_allow_html=True)

# Основные настройки — всё на русском.
...
control[2].checkbox("Подогнать текст под тайминг", value=True)
control[3].checkbox("Очистить шум / эхо", value=True)

left, mid, right = st.columns([1, .10, 1], gap="small")

with left:
    st.markdown("<div class='rowbox'><h3>⬅️ 1. Вставить видео</h3>", unsafe_allow_html=True)
    video_file = st.file_uploader("Загрузить видео из папки", type=["mp4", "mov", "webm", "mkv"], key="video_file")
    ...
    st.markdown("<div class='rowbox'><h3>⬅️ 3. Вставить ссылку</h3>", unsafe_allow_html=True)
...
with right:
    ...
    st.caption("Preview показывает место результата. На VPS здесь будет финальный MP4.")
    st.markdown("</div>", unsafe_allow_html=True)
...
st.caption("Murat AI preview. Все подписи на русском. Реальное скачивание платформ, AI-клонирование голоса и финальный MP4 требуют VPS/GPU backend.")
